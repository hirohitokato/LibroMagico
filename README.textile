h1. LibroMagico

Execute bookmarklets from right click.

This is a Safari version of Takayuki Fukatsu's SpellBook.

* "SpellBook":https://chrome.google.com/extensions/detail/ihckioenbbjedpocnnennnehjaacojil -- a great extension for Chrome!

He says:
_Bookmarklet is like a magic spell, collect them and build your own SpellBook!_

Let's check "marklets":http://marklets.com/ .

h2. Installing LibroMagico

# Install "SIMBL":http://www.culater.net/software/SIMBL/SIMBL.php
# Put LibroMagico.bundle into @~/Library/Application Support/SIMBL/Plugins@ folder
** You can download the file from "http://cielo.rojo.jp/files/LibroMagico.zip":http://cielo.rojo.jp/files/LibroMagico.zip .
# Relaunch a Safari

h2. Uninstalling LibroMagico

# Remove LibroMagico.bundle from @~/Library/Application Support/SIMBL/Plugins@ folder

just it!

h2. Usage

# Put your bookmarklets into 'Bookmarklets' folder.
** *Bookmarklets folder must be in Bookmark Bar*.
# You can execute them from right click.

h2. Special Thanks
* Takayuki Fukatsu (a.k.a. @fladdict) "his site":http://artandmobile.com/
* moyashi (a.k.a. @hitoriblog) "his site":http://hitoriblog.com/